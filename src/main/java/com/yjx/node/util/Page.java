...
public class Page<T> {
    ...
    private Integer totalPage; //总页数  （总记录数/每页显示的数量；将参数转换为浮点型，执行除法操作，向上取整）
    ...
    public Page(Integer pageNum, Integer pageSize, long totalCount) {
        ...
        this.nextPage = pageNum+1 >totalCount ? totalPage : pageNum+1;

        //导航开始页（当前页-5）
        this.startNavPage = this.pageNum-5;
        //导航结束页 (当前页+4)
        this.endNavPage = this.pageNum+4;
        ...
        if (this.startNavPage < 1){
            ...
            this.endNavPage = startNavPage+9 > totalPage ? totalPage : startNavPage+9;
        }
        // 导航结束页
        if (this.endNavPage > totalPage){
            ...
            this.startNavPage = endNavPage - 9 < 1 ? 1: endNavPage-9;
        }

...
import com.yjx.node.po.NoteType;
import com.yjx.node.po.User;
import com.yjx.node.service.NoteService;
import com.yjx.node.service.NoteTypeService;
...
public class NoteServlet extends HttpServlet {
     ...
     * @param response
     */
    private void noteDetlete(HttpServletRequest request, HttpServletResponse response) throws IOException {
        //1. 接收参数 （noteId）
        String noteId = request.getParameter("noteId");
        //2. 调用Service层删除方法，返回状态码 （1=成功，0=失败）
        Integer code = noteService.noteDetlete(noteId);
        //3. 通过流将结果响应给ajax的回调函数 （输出字符串）
    ...
    private void noteDetail(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //4. 设置首页动态包含的页面值
        request.setAttribute("changePage","node/detail.jsp");
        //5. 请求转发跳转到index.jsp
    ...
    private void noteView(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //4.设置首页动态包含的页面值
        request.setAttribute("changePage","node/view.jsp");
        //5.请求转发跳转到index.jsp
        request.getRequestDispatcher("index.jsp").forward(request,response);
    }
}
